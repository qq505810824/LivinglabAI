...
export function TeaTalkCard({ talk, onLike, onDislike }: TeaTalkCardProps) {
  ...
  return (
    <>
      <div className="bg-white rounded-xl shadow-sm border border-earth-100 flex overflow-hidden hover:shadow-md transition-shadow duration-300">
        {/* Left: User Info */}
        <div className="flex-shrink-0 flex flex-col items-center w-24 p-6 border-r border-earth-50 bg-tea-50/30">
          <div className="w-12 h-12 rounded-full overflow-hidden bg-earth-100 mb-2 ring-2 ring-white shadow-sm">
             {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={talk.user.avatar} alt={talk.user.name} className="w-full h-full object-cover" />
          </div>
          <h3 className="font-medium text-tea-900 text-sm text-center break-words w-full leading-tight">
            {talk.user.name}
          </h3>
        </div>

        {/* Middle: Content */}
        <div className="flex-grow p-5 flex flex-col min-w-0">
          <div>
            {/* Title */}
        ...
        {talk.image_url && (
          <div 
            className="flex-shrink-0 w-48 relative cursor-zoom-in group"
