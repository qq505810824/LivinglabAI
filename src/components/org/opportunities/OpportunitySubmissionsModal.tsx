...
export function OpportunitySubmissionsModal({
  opportunity,
  ownerId,
  onClose,
}: OpportunitySubmissionsModalProps) {
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <h2 className="text-lg font-semibold text-text-primary">
            Applications for: <span className="font-bold">{opportunity.title}</span>
          </h2>
          <button
                ...
                <tbody>
                  {submissions.map((s) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 align-top text-xs text-text-secondary">
                        {s.university ?? s.payload?.university ?? '—'}
                      </td>
