...
import {
  formatInternshipApplicationFields,
  formatProgramApplicationFields,
} from '@/utils/submissionDisplay';

interface OpportunitySubmissionDetailModalProps {
  submission: Submission;
  onClose: () => void;
}

export function OpportunitySubmissionDetailModal({
  submission,
  onClose,
}: OpportunitySubmissionDetailModalProps) {
  const isInternship = submission.resource_type === 'internship_application';
  const fields = isInternship
    ? formatInternshipApplicationFields(submission)
    : formatProgramApplicationFields(submission);

  return (
    <div
      className="fixed inset-0 z-60 flex items-center justify-center bg-black/40 px-4 animate-in fade-in-0"
      onClick={(e) => {
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <h2 className="text-lg font-semibold text-text-primary">Application Detail</h2>
          <button
        ...
        <div className="px-6 py-4 max-h-[60vh] overflow-y-auto text-xs space-y-3">
          <div>
            <div className="font-semibold text-text-secondary mb-1">Applicant</div>
